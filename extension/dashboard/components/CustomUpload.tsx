...
export default function CustomUpload({base,project,refreshKey,onChanged,onError}:{base:string;project:Project;refreshKey:number;onChanged:(m:string)=>void;onError:(m:string)=>void}){
    const[busy,setBusy]=useState(false),[name,setName]=useState(''),[destination,setDestination]=useState('mods/custom.jar'),[destinationEdited,setDestinationEdited]=useState(false);
    ...
    const upload=async(e:React.FormEvent<HTMLFormElement>)=>{
        ...
        try{
            ...
            form.reset();
            setName('');setDestination('mods/custom.jar');setDestinationEdited(false);
    ...
    return <div className="pwm-grid">
        <Card><SectionHeading icon="upload" title="Custom JAR" description="Upload a trusted mod that is not available through the configured providers."/>
            ...
            <form onSubmit={upload} className="pwm-form-grid" style={{marginTop:12}}>
                ...
                <div className="pwm-field"><label className="pwm-label">Mod name</label><Input name="display_name" value={name} onChange={e=>changeName(e.currentTarget.value)} placeholder="My Mod" required/></div>
