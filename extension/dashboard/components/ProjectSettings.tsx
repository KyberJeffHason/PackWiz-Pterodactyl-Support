...
export default({base,project,reload}:{base:string;project:Project;reload:()=>void})=>{
    ...
    return <div className="pwm-grid" style={{gap:14}}><Card><SectionHeading icon="settings" title="Project settings" description="Update pack metadata. Packwiz is re-initialized with these values while preserving the managed project tree." actions={<Pill><Icon name="package" size={13}/>{project.slug}</Pill>}/>
    ...
    </Card><Card muted><SectionHeading icon="info" title="Project identity" description="Stable identifiers are intentionally not editable because published URLs and integrations reference them."/><div className="pwm-kv"><div className="pwm-kv-key">Project ID</div><div className="pwm-kv-value pwm-code">{project.id}</div><div className="pwm-kv-key">Slug</div><div className="pwm-kv-value pwm-code">{project.slug}</div><div className="pwm-kv-key">Published revision</div><div className="pwm-kv-value">{project.current_revision||'Not published yet'}</div></div></Card></div>;
};
