...
func safePublicPathPart(part string) bool {
	if part == "" || part == "." || part == ".." {
		return false
	}
...
func PublicHandler(releasesRoot, blobsRoot string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var name string
		immutable := false
		if len(parts) >= 4 && parts[0] == "blobs" && parts[1] == "sha256" && len(parts[2]) == 64 && publicPart.MatchString(parts[2]) && safePublicPathPart(parts[3]) {
			name = filepath.Join(blobsRoot, "sha256", parts[2][:2], parts[2])
			immutable = true
		} else if len(parts) >= 2 && publicPart.MatchString(parts[0]) {
			for _, p := range parts[1:] {
				if !safePublicPathPart(p) {
					http.NotFound(w, r)
					return
				}
			}
