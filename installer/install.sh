...
VERSION="latest"; REPO="packwiz-manager/packwiz-manager"; SERVICE_ONLY=0; COMPOSE_FILE=""; EXTENSIONS_DIR=""; PACK_HOST=""
usage(){ echo "Usage: install.sh [--version v0.2.0] [--repo owner/repo] [--service-only] [--compose-file FILE] [--extensions-dir DIR] [--pack-host URL]"; }
while (($#)); do case "$1" in --version) VERSION="$2";shift 2;;--repo) REPO="$2";shift 2;;--service-only) SERVICE_ONLY=1;shift;;--compose-file) COMPOSE_FILE="$2";shift 2;;--extensions-dir) EXTENSIONS_DIR="$2";shift 2;;--pack-host) PACK_HOST="$2";shift 2;;--configure-cloudflare) echo "Cloudflare configuration requires manual scoped-token workflow; existing ingress is never modified by this installer." >&2;exit 2;;-h|--help)usage;exit;;*)echo "Unknown option: $1" >&2;usage;exit 2;;esac;done
